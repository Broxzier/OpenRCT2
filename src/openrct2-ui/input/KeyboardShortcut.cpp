...
void keyboard_shortcut_handle(int32_t key)
{
    ...
    static const char* window_names[] = {
        ...
        "Editor Invention List",
        "Editor Invention List Drag",
        "Editor Scenario Options",
        "Edtior Objective Options",
        "Manage Track Design",
        "Track Delete Prompt",
        "Install Track",
        "Clear Scenery",
        ...
        "unknown",
        "Staff",
        "Editor Track Bottom Toolbar",
        "Editor Scenario Bottom Toolbar",
        "Chat",
        "Console",
    };

    auto windowIsSticky = [](const rct_window* window) -> bool {
        return window->flags & (WINDOW_FLAGS::WF_STICK_TO_BACK | WINDOW_FLAGS::WF_STICK_TO_FRONT);
    };
    auto windowIsNonSticky = [&windowIsSticky](const rct_window* window) -> bool { return !windowIsSticky(window); };

    std::puts("=== " __FUNCTION__ " ===");
    auto it = std::find_if(g_window_list.rbegin(), g_window_list.rend(), [&](std::shared_ptr<rct_window> window) -> bool {
        const int name = window->classification;
        std::printf("%s\t(%s)\n", window_names[name], windowIsSticky(window.get()) ? "sticky" : "non-sticky");

        // Skip sticky windows
        if (windowIsSticky(window.get()))
        {
            return false;
        }

        if (window->event_handlers->key_down == nullptr)
        {
            return false;
        }

        return window->event_handlers->key_down(window.get(), key);
    });
    if (it != g_window_list.rend())
    {
        // Event has been handled.
        std::puts("Key down event has been handled.");
        return;
    }

    // First attempt to let non-sticky windows try to catch the event

    // Then attempt to let the sticky windows catch the event

    // Then attempt to use the shortcut table function list
